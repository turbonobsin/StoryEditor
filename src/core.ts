...
class Board extends StoryObj{
    ...
    update(): void {
        ...
        else this.div.classList.remove("has-img");
    ...
    setAudio(name:string,noEmit=false){
        this.audio = name;
        if(name == null){
            if(_editBoard_b == this){
        ...
        if(!noEmit) socket.emit("s_setBGAudio",this._id,name,(code:number)=>{
            if(code == 0){
...
function loadEditBoard(b:Board){
    ...
    if(b.story){
        if(b.story.start == b) b_makeStart.disabled = true;
        else b_makeStart.disabled = false;
    }
}
