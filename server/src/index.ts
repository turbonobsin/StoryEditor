...
class User{
    ...
    async openProject(email:string,name:string){
...
async function getWho(socket:Socket){
    ...
    u = await new Promise<User>(async resolve=>{
        fs.readFile("users/"+email+".json",{encoding:"utf8"},(err,data)=>{
            ...
            if(err) resolve(null);
            else resolve(User.from(JSON.parse(data),socket));
        });
    });
    return u;
}
async function getUserByEmail(email:string){
    ...
    return await new Promise<User>(async resolve=>{
        fs.readFile("users/"+email+".json",{encoding:"utf8"},(err,data)=>{
            // console.log("found",email,data);
            if(err) resolve(null);
            else resolve(User.from(JSON.parse(data),null));
        });
    });
}
async function getProject(email:string,pname:string){
    ...
    p = await new Promise<Project>(async resolve=>{
        fs.readFile("projects/"+email+"/"+pname+"/"+"meta.json",{encoding:"utf8"},async (err,data)=>{
            if(err) resolve(null);
            else resolve(Project.from(JSON.parse(data),await getUserByEmail(email)));
        });
    });
    if(!p) return;
...
io.on("connection",socket=>{
    ...
    socket.on("createProject",async (name:string)=>{
        let u = await getWho(socket);
        if(!u) return;
        await u.createProject(name);
    });
    socket.on("openProject",async (email:string,name:string,f:(data:any)=>void)=>{
        let u = await getWho(socket);
        if(!u) return;
    ...
    socket.on("s_addChoice",async (id:number,labels:string[])=>{
        let u = await getWho(socket);
        if(!u) return;
        let p = u.curProject;
        if(!p) return;
